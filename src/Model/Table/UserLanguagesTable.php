...
class UserLanguagesTable extends Table
{
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ->scalar('proficiency_level')
            ->maxLength('proficiency_level', 200)
            ->requirePresence('proficiency_level', 'create')
            ->notEmpty('proficiency_level');
        ...
        $validator
            ->scalar('write')
            ->maxLength('write', 200)
            ->requirePresence('write', 'create')
            ->notEmpty('write');

        $validator
            ->scalar('speak')
            ->maxLength('speak', 200)
            ->requirePresence('speak', 'create')
            ->notEmpty('speak');
