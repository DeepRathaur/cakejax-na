...
<div class="customerDocuments view large-9 medium-8 columns content">
    <h3><?= h($customerDocument->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Customer') ?></th>
            <td><?= $customerDocument->has('customer') ? $this->Html->link($customerDocument->customer->title, ['controller' => 'Customers', 'action' => 'view', $customerDocument->customer->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Login Id') ?></th>
            <td><?= h($customerDocument->login_id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Customer Documents') ?></th>
            <td><?= h($customerDocument->customer_documents) ?></td>
        </tr>
        <tr>
